...
test("OpenCode maps host events to the same canonical fields with its agent type", () => {
  const record = mapHostEvent("opencode", event);
  expect(record.agent_type).toBe("opencode");
...
describe("stdin adapter", () => {
  test("emits one success result per submitted event", async () => {
    ...
    await runAdapter("opencode", Readable.from(`${JSON.stringify(event)}\n`), { write: (value: string) => { outputs.push(value); return true; } }, bridge);
  ...
  test("emits one failure for one malformed event", async () => {
    const outputs: string[] = [];
    await runAdapter("opencode", Readable.from('{"sessionId":\n'), { write: (value: string) => { outputs.push(value); return true; } });
    expect(outputs).toHaveLength(1);
