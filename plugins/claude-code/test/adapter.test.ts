...
test("Claude Code maps host events to the canonical source-free record", () => {
  const record = mapHostEvent("claude-code", event);
  expect(record.agent_type).toBe("claude-code");
  expect(record.session_id).toBe(event.sessionId);
  expect(record.targets[0]?.path).toBe("src/change.ts");
  expect(record.targets[0]).not.toHaveProperty("source_body");
});

test("Claude Code emits one result for one valid event", async () => {
  ...
  expect(JSON.parse(outputs[0] ?? "").recordId).toBe("shared-record");
});
