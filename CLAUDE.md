...
The Cursor equivalent is `ai-code-review-cursor` (`plugins/cursor`) plus the committed `.cursor/hooks.json`. `preToolUse` / `beforeShellExecution` reject `Write`/`StrReplace`/`ApplyPatch`/`Delete` and shell mutations unless a matching permit exists. Record with the bundled MCP tool `review_record_judgment` (or `bun plugins/cursor/bin/adapter.mjs record`). Session setup is automatic via `sessionStart` and `beforeSubmitPrompt`, which export `AI_REVIEW_SESSION_ID`, `AI_REVIEW_REPOSITORY_ROOT`, and `AI_REVIEW_AGENT_TYPE=cursor`. Manual fallback: `ai-review setup --root "$PWD" --agent-type cursor`. A project-only copy under `.cursor/plugins/` must keep the gate on that project's `.cursor/hooks.json` and omit `hooks` from the copied `plugin.json`; otherwise Cursor also loads the bundled fail-closed hooks with a stale installPath and blocks Write/Shell with `spawn /bin/zsh ENOENT`.
